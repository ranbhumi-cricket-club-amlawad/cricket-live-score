...
The public scorecard is shown while `currentMatch.status` is `LIVE` or `INNINGS BREAK`. Set a prepared current match to `PRE LIVE` to hide its live scorecard and show it again in the Upcoming Matches section without losing admin setup or score data. During an innings break it keeps the full scorecard visible with a clear break banner. Upcoming and completed/cancelled matches have separate sections. Select an upcoming match card to view both team squads, or open a completed match to view player run details and bowler over details.
